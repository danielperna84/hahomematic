...
async def test_cecover(
    ...
    cover: CeCover = cast(
        CeCover, await helper.get_custom_entity(central, "VCU8537918", 4)
    )
...
async def test_ceblind(
    ...
    central, mock_client = await central_local_factory.get_default_central(TEST_DEVICES)
    cover: CeBlind = cast(
        CeBlind, await helper.get_custom_entity(central, "VCU0000145", 1)
    )
    assert cover.usage == HmEntityUsage.CE_PRIMARY
    assert cover.channel_operation_mode is None
    assert cover.current_cover_position == 0
...
async def test_ceipblind(
    ...
    cover: CeIpBlind = cast(
        CeIpBlind, await helper.get_custom_entity(central, "VCU1223813", 4)
    )
    assert cover.usage == HmEntityUsage.CE_PRIMARY
...
async def no_test_cegarage(
    ...
    central, mock_client = await central_local_factory.get_default_central(TEST_DEVICES)
    cover: CeGarage = cast(
        CeGarage, await helper.get_custom_entity(central, "VCU3574044", 1)
    )
    assert cover.usage == HmEntityUsage.CE_PRIMARY
