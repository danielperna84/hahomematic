...
class Example:
    ...
    def _systemcallback(self, name, *args, **kwargs):
        ...
        if name == const.HH_EVENT_NEW_DEVICES and kwargs \
                and kwargs.get('device_descriptions') and len(
                kwargs['device_descriptions']) > 0:
            self.got_devices = True
            return
        if name == const.HH_EVENT_DEVICES_CREATED and kwargs \
                and kwargs.get('new_devices') and len(
                kwargs['new_devices']) > 0:
